...
- 四个平台分别使用独立 Prompt Profile 和 Pydantic 结构化输出模型；
...
- 微博、X 与微信公众号只允许真实官方接口连接；未通过官方验证时禁止创建发布任务；
...
2. 在“内容库”新建或导入一篇原始文章。
3. 进入“创作”，选择微博、X、小红书和微信公众号中的一个或多个平台。
4. 选择“快速改写”或“深度创作”，再设置风格、长度、目标受众、原意保留程度、Emoji 和标签偏好。
5. 点击生成。四个平台会并行处理，并分别显示实时进度和失败原因。深度创作还会显示切入角度、开场钩子、候选标题和 AI 选稿结果。
...
平台发布不提供模拟成功模式。微博、X 和微信公众号使用官方接口，小红书默认采用明确标记的人工交付流程。所有非官方本地连接均为显式开启的实验功能，关闭时不会加载、测试或调用。
...
- 两项本地连接默认均关闭，不会生成虚假的平台 ID，也不会把“连接测试通过”伪装成“平台发布成功”。
...
# 微信公众号：安装并登录 @wechatsync/cli 后，允许官方接口失败时同步草稿
WECHATSYNC_CLI_ENABLED=true
```

启用小红书开关后，在“平台账号”选择“本机小红书 MCP 试运行”，扫码登录并通过连接检测，排期页才会显示可用的 MCP 发布方式。当前试运行固定使用“仅自己可见”，避免未经确认就公开发文；任一步未完成时仍默认人工发布。Wechatsync 只作为草稿同步后备，不替代官方凭证配置。
