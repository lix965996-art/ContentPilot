...
import type { Article, ArticleEngagementSummary, Schedule } from '@/types/business'
...
const recentEngagement = ref<Record<number, ArticleEngagementSummary>>({})
...
function openArticle(article: Article) {
  ...
  router.push({ name: 'articles', query: { edit: article.id } })
}
...
async function load() {
  ...
  try {
    ...
    const engagementData = await workflowApi.articlesEngagement(
      articlesData.items.map((item) => item.id),
    )
    ...
    recentEngagement.value = engagementMap
        ...
        <section class="workspace-section recent-content">
          ...
          <div v-if="recentArticles.length" class="recent-list">
            <article v-for="item in recentArticles" :key="item.id">
              <button type="button" class="recent-cover" @click="openArticle(item)">
                <img
              ...
              <button class="recent-copy" @click="openArticle(item)">
                <b>{{ item.title }}</b
                ><small>{{ item.summary || item.sourceText }}</small>
                <EngagementMetricsBar
                  v-if="recentEngagement[item.id]?.totals"
                  compact
                  :totals="recentEngagement[item.id]?.totals"
                  :simulated="recentEngagement[item.id]?.simulated"
                />
              </button>
