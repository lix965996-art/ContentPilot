...
import { FilePlus2, MoreHorizontal, PenLine, Search, Upload } from 'lucide-vue-next'
...
function requestRemove(row: Article) {
  deleteTarget.value = row
  confirmDeleteOpen.value = true
}
async function confirmRemove() {
  if (!deleteTarget.value) return
  await workflowApi.deleteArticle(deleteTarget.value.id)
...
async function handleRouteAction() {
  if (route.query.create === '1' && canOperate.value) openCreate()
  const editId = Number(route.query.edit)
  if (editId) {
    const row = rows.value.find((item) => item.id === editId)
    if (row) await openEdit(row)
  }
}
watch(
  () => [route.query.create, route.query.edit],
...
<template>
  <div>
      ...
      <article v-for="row in rows" :key="row.id" class="content-row">
        ...
        <div v-if="canOperate" class="row-actions">
          <el-tooltip content="继续编辑"
            ><button class="continue-edit" @click="toStudio(row)">
              <PenLine :size="14" /><span>继续</span>
            </button></el-tooltip
          ><el-dropdown trigger="click"
            ...
            ><template #dropdown
              ><el-dropdown-menu
                ...
                ><el-dropdown-item divided @click="requestRemove(row)"
                  >删除</el-dropdown-item
                ></el-dropdown-menu
              ></template
            ></el-dropdown
    ...
    <ConfirmDialog
      ...
      title="删除内容"
      :message="`确认删除《${deleteTarget?.title || ''}》？此操作无法撤销。`"
      danger
      @confirm="confirmRemove"
    />
    <el-dialog
      v-model="editorOpen"
      :title="!canOperate ? '查看原文' : editingId ? '编辑原文' : '新建内容'"
    ...
    >
      <el-form label-position="top"
        ><div class="grid gap-x-4 sm:grid-cols-2">
          <el-form-item label="标题" class="sm:col-span-2" required
            ><el-input v-model="form.title" :disabled="!canOperate" maxlength="255" show-word-limit
          /></el-form-item>
          <el-form-item label="主题"
            ><el-input
              v-model="form.topic"
              :disabled="!canOperate"
              placeholder="例如：校园运营" /></el-form-item
          ><el-form-item label="目标受众"
            ><el-input v-model="form.target_audience" :disabled="!canOperate"
          /></el-form-item>
          <el-form-item label="语气"
            ><el-select v-model="form.tone" :disabled="!canOperate" class="w-full"
          ...
          <el-form-item label="摘要" class="sm:col-span-2"
            ><el-input v-model="form.summary" :disabled="!canOperate" type="textarea" :rows="2"
          /></el-form-item>
      ...
      ></el-form>
      <template #footer
        ><el-button @click="editorOpen = false">{{ canOperate ? '取消' : '关闭' }}</el-button
        ><el-button v-if="canOperate" type="primary" @click="save">保存内容</el-button></template
      >
    </el-dialog>
  </div>
</template>
