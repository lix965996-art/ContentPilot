...
def test_mock_connection_and_oauth_state(client, login_as) -> None:
    auth = _operator_auth(client, login_as)
...
def test_wechat_mock_draft_is_not_public_success(client, login_as) -> None:
    auth = _operator_auth(client, login_as)
    ...
    assert data["status"] == "MOCK_DRAFT_CREATED"
    assert data["resultMode"] == "MOCK"
    assert not data["publishedUrl"]
    assert data["externalId"].startswith("mock-draft-")
...
def test_real_publish_is_forbidden_for_mock_account(client, login_as) -> None:
    auth = _operator_auth(client, login_as)
    configured = client.put(
        "/api/platform-accounts/WECHAT_OFFICIAL",
        headers=auth,
        json={
            "account_name": "仅 Mock 公众号",
            "auth_type": "APP_SECRET",
            "publish_mode": "MOCK",
            "app_id": "wx-mock-only",
        },
